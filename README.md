...
Helix Framework AI is a local frontend/backend app for helping a science department compare teacher-uploaded documents to the Science Department OS Framework, rate departments/courses/assignments, and generate actionable PLC recommendations.
...
The upload endpoints accept `multipart/form-data` with one or more `files` fields plus optional `scope`, `course`, and `gradeBand` fields. Supported upload text extraction includes TXT/CSV, best-effort PDF, best-effort DOCX, and JSON.
